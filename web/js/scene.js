  ...
  _recolour() {
    ...
    for (let q = 0; q < this.nQuad; q++) {
      ...
      if (layer === 'priority' || layer === 'annual_priority') {
        ...
        c = f(norm(v !== undefined ? v : NaN, 0, 85));
      } else if (annualArr) {
        ...
        c = f(norm(annualArr[p * nBand + b], annualDom[0], annualDom[1]));
      } else if (layer === 'air') {
        c = f(norm(d.airAt(this.hour, p, b), dom[0], dom[1]));
      } else if (layer === 'sun') {
        c = d.sunlitAt(this.hour, p, b) ? SUNLIT_RGB : SHADE_RGB;
      } else if (durField) {
        c = f(norm(durSample[p], durDom[0], durDom[1]));
      } else {
